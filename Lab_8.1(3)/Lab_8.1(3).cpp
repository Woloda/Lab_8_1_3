#include <iostream>
#include <Windows.h>

using namespace std;

char* perevirka(char* lit_r, const char* para, const char* para1, const char* zamina);

int main()
{
	SetConsoleCP(1251);
	SetConsoleOutputCP(1251);

	char lit_r[50] = "QhkiQQSuySQQsdQSdsSQsdQQQSSSq";
	cout << "Сфомований літерний рядок: " << endl << endl << lit_r << endl << endl;
	char para[] = "SQ";
	char para1[] = "QS";
	char zamina[] = "***";

	cout << "Модифікований літерний рядок: " << endl << endl;
	cout << perevirka(lit_r, para, para1, zamina) << endl;

	return 0;
}

char* perevirka(char* lit_r, const char* para, const char* para1, const char* zamina)
{
	char* d = new char[strlen(lit_r) / 2 * 3 + 1];
	char* t = d;
	int j, k, l = 0;
	for (int i = 0; lit_r[i] != '\0'; i++)
	{
		j = 0;
		k = i;
		l = 0;
		while (para[j] != '\0')
		{
			if (((lit_r[k] == para[j]) || (lit_r[k] == para1[j])) && lit_r[k] != '\0')
			{
				if (j == 1)
					l++;
				j++;
				k++;
			}
			else
				break;
		}
		if (para[j] == '\0' && (lit_r[k - 1] != lit_r[k - 2]))
		{
			*t++ = '*';*t++ = '*';*t++ = '*';
			i = k - 1;
		}
		else
		{
			if (l == 1)
			{
				*t++ = lit_r[k - 2];
				i = k - 2;
			}
		}
		if (j == 0)
		{
			*t++ = lit_r[k];
			i = k;
		}
		if (j == 1)
		{
			*t++ = lit_r[k - 1];
			*t++ = lit_r[k];
			i = k;
		}
	}
	*t = '\0';
	strcpy_s(lit_r, strlen(lit_r) / 2 * 3 + 1, d);
	return d;
}
